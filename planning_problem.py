    ...
    def get_successors(self, state):
        ...
        for action in self.actions:
            if action.all_preconds_in_list(state):
                ...
                list_of_triples.append((frozenset(successor), action, step_cost))
...
def max_level(state, planning_problem):
    ...
    while not planning_problem.is_goal_state(pg_init.proposition_layer.propositions):
        if prev_state == pg_init.proposition_layer.propositions:
            return float('inf')
        prev_state = pg_init.proposition_layer.propositions
        state_max_level += 1
        pg_init.expand_without_mutex(pg_init)  # ?
...
def level_sum(state, planning_problem):
    ...
    while len(goal_dict) < len(goals):
        ...
        pg_init.expand_without_mutex(pg_init)
        for goal in goals:
            if goal in pg_init.proposition_layer.propositions and goal not in goal_dict: # new proposition
                goal_dict[goal] = level

    return sum(goal_dict.values())
