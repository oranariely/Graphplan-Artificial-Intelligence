...
class GraphPlan(object):
    ...
    def graph_plan(self):
        ...
        level = 0
        self.no_goods = []  # make sure to update noGoods in your backward search!
...
def independent_pair(a1, a2):
    ...
    a1.is_pos_effect(p) returns true if p is in a1.get_add()
    a1.is_neg_effect(p) returns true if p is in a1.get_delete()
    """
    add1 = a1.get_add()
    add2 = a2.get_add()
    del1 = a1.get_delete()
    ...
    pre_con1 = a1.get_pre()
    pre_con2 = a2.get_pre()
    '''inconsistent effects: one action adds a proposition that the other action deletes '''
    # p1 is a change in a1 but being deleted by a2
    for p1 in add1:
        if p1 in del2:
            return False
    # p2 is a change in a1 but being deleted by a1
    for p2 in add2:
        if p2 in del1:
            return False
    '''Inference: one action deletes the precondition of another '''
    # d1 is deleted by a1 but is a pre condition of a2
    for d1 in del1:
        if d1 in pre_con2:
            return False
    # d2 is deleted by a1 but is a pre condition of a1
    for d2 in del2:
        if d2 in pre_con1:
            return False
    return True
